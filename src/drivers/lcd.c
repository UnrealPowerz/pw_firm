...
/* Reason: not a real function -- compiler runtime helper.
 * The ROM bytes at 0x7a0a are the entry-side of an inline register-save
 * thunk (subs #2,er7; push er3..er6; juggle return address; rts).  The
 * decompiler attached the symbol drv_lcd_strobe_res to it because of the
 * surrounding LCD code, but it is generated by the compiler's calling
 * convention, not user C.  Cannot be reproduced in C.
 * Class: compiler-runtime-helper */
// ROM: 0x7a0a  11.1%
void drv_lcd_strobe_res(void) {}

/* Reason: not a real function -- compiler runtime helper.
 * The ROM bytes at 0x7a2a are the exit-side of the matching register-save
 * thunk (pop er2..er6; rts).  Same situation as drv_lcd_strobe_res -- the
 * decompiler labelled it but it is compiler-emitted, not user C.  The C
 * body below happens to be the *real* "cs high" code that lives elsewhere
 * in the ROM; this stub will never byte-match.
 * Class: compiler-runtime-helper */
// ROM: 0x7a2a  51.7%
