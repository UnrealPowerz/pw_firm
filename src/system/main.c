...
void sys_main_loop_low_power(void) {
  ...
  if (!(g.sys_walkerFlags.BYTE & WALKER_MODE_MASK)) {
    game_dispatch_pedometer_task();
    if (game_detect_activity()) {
      sys_enter_low_power();
    } else if (g.sys_statusFlags.BIT.button_event) {
      sys_enter_low_power();
    }
  } else if ((g.sys_walkerFlags.BYTE & WALKER_MODE_MASK) == WALKER_MODE_DEEP_SLEEP) {
