...
/* Sub-animation index in g.gCurSubstateY for VIEW_WALK_DEPARTURE_ANIM. */
...
void ui_draw_cloud_rise_anim(void) {
  ...
  if (g.gCurSubstateZ <= 4) {
    ...
    drv_lcd_blit(
        0x2c, ANIM_CLOUD_Y[g.gCurSubstateZ],
        (void *)ptr, 8, 8);
  }
...
phase_start:
  ...
  g.gCurSubstateZ = 0;
  g.gCurSubstateY = DEPARTURE_POKE;
  goto sound;
phase_poke:
  if (z <= 8) goto done;
  g.gCurSubstateZ = 0;
  g.gCurSubstateY = DEPARTURE_CLOUD_AFTER;
  goto done;
phase_cloud:
  if (z < 9) goto done;
  y = DEPARTURE_SUCCESS;
