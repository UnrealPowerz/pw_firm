 ...
 *
 * Cursor is `menu_cursor` (0..5, see enum main_menu_item in
 * include/menu_consts.h). gCurSubstateY tracks any overlay popup (see
...
/* Main-menu overlay popups (gCurSubstateY in ui_handle_main_menu /
   ui_render_main_menu). NONE = normal cursor/watts display. */
...
void ui_handle_main_menu(void) {
  ...
  if (drv_button_is_triggered(BTN_R)) {
    ...
    if (watts < cost) {
      gCurSubstateY = MENU_POPUP_NEED_WATTS;
      drv_sound_play(SND_CURSOR);
      return;
    }

    if (menu_cursor > MENU_SETTINGS) {
    } else {
      switch (menu_cursor) {
      case MENU_POKERADAR:
        if (!(walker_status_flags_BIT.walking)) {
          gCurSubstateY = MENU_POPUP_NO_POKEMON;
          drv_sound_play(SND_CURSOR);
          return;
        }
        cost = costTable[menu_cursor];
      ...
      case MENU_INVENTORY: {
        ...
        gCurSubstateY = MENU_POPUP_NOTHING_HELD;
        drv_sound_play(SND_CURSOR);
        return;
      }
      case MENU_SETTINGS:
...
void ui_render_main_menu(void) {
  ...
  if (gCurSubstateY == MENU_POPUP_NONE) {
    ...
    } else if (menu_cursor == MENU_DOWSING) {
    ...
    if (menu_cursor < MENU_CONNECTION) {
