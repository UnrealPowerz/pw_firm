 ...
 * (returns from a session). Sub-animation index in gCurSubstateY drives the
 * phase dispatch.
 *
...
/* Sub-animation index in gCurSubstateY for VIEW_WALK_ARRIVAL_ANIM. */
...
phase_cloud:
  ...
  if (z == 0) goto done;
  gCurSubstateY = ARRIVAL_POKE;
  gCurSubstateZ = 0;
  goto play;
phase_poke:
  if (z <= 8) goto done;
  gCurSubstateZ = 0;
  gCurSubstateY = ARRIVAL_SUCCESS;
play:
