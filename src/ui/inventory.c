 ...
 *   show. The two words are stashed in (gCurSubstateA:uint16) and
 *   accelPos_X:uint16, then read back by the render functions.
 *
 *   mask[0]  (held in *(uint16_t *)&gCurSubstateA, the pokemon/event word):
 ...
 * _back_wrap / _reset) advance gCurSubstateY through the 10-bit presence
 * mask, skipping empty slots.
 */

/* Inventory grid slot indices for gCurSubstateY. */
enum inventory_slot {
...
void ui_inventory_jump_to_items(void) {
  gCurSubstateY = INV_SLOT_EVENT_ITEM;
...
void ui_handle_inventory_pokemon(void) {
  ...
  if (drv_button_is_triggered(BTN_M) != 0) {
    if (ui_inventory_cursor_prev(*(volatile uint16_t *)&gCurSubstateA) != 0) {
  ...
  if (drv_button_is_triggered(BTN_L) != 0) {
    if (ui_inventory_cursor_next(*(volatile uint16_t *)&gCurSubstateA) != 0) {
...
void ui_handle_inventory_items(void) {
  ...
  if (drv_button_is_triggered(BTN_M) != 0) {
    if (ui_inventory_cursor_prev(accelPos_X) != 0) {
      ...
      if (gCurSubstateA != 0) {
        gCurSubstateY = 0;
        ui_inventory_cursor_back_wrap(*(volatile uint16_t *)&gCurSubstateA);
...
void ui_render_inventory_pokemon(void) {
  ...
  if (gCurSubstateY == INV_SLOT_OWN_POKE)
    x -= 8;
...
void ui_inventory_cursor_reset(void) {
  gCurSubstateY = INV_SLOT_EVENT_ITEM;
  ui_inventory_cursor_find_present(*(uint16_t *)&gCurSubstateA);
}
