...
void ui_render_battle(void) {
  ...
  if (g.viewstate.Z <= BS_THROW_BALL) {
    uint8_t kind = g.viewstate.Y.BYTE;
    if (kind < 4) {
...
void game_battle_handle_finish(void) {
  uint8_t kind_idx = (uint8_t)(g.viewstate.Y.BYTE - 1);
  ...
  if (kind_idx < 3) {
    ...
    gift_buf = sbrk(0x88);
    game_log_interaction(trainer_buf, gift_buf, 0x0D, 0, 0, (uint8_t)g.viewstate.Y.BYTE);
  } else {
    ...
    game_log_interaction(trainer_buf, gift_buf, 0x0E, 0x01, 0, (uint8_t)g.viewstate.Y.BYTE);
  }
}
...
void ui_handle_battle(void) {
  ...
  case BS_DEFEATED:
      ...
      return;
    {
      ...
      if (g.viewstate.Y.BYTE < 4) {
        log_buf = sbrk(0x10);
        game_log_interaction(log_buf, trainer_buf, 0x10, 0x00, 0, (uint8_t)g.viewstate.Y.BYTE);
      } else {
        log_buf = sbrk(0x110);
        game_log_interaction(log_buf, trainer_buf, 0x10, 0x01, 0, (uint8_t)g.viewstate.Y.BYTE);
      }
      if (g.save_watts >= 10) {
  ...
  case BS_FLED:
    ...
    if (drv_button_is_triggered(BTN_ANY)) {
      ...
      if (g.viewstate.Y.BYTE < 4) {
        log_buf = sbrk(0x10);
        game_log_interaction(log_buf, trainer_buf, 0x0F, 0x00, 0, (uint8_t)g.viewstate.Y.BYTE);
      } else {
        log_buf = sbrk(0x110);
        game_log_interaction(log_buf, trainer_buf, 0x0F, 0x01, 0, (uint8_t)g.viewstate.Y.BYTE);
      }
